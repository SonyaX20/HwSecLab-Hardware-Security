...
# Assuming key_guess_length is the length of your key in bytes
...
for i in range(5):        
    ...
    with open('./data/collected_data_'+str(i+1)+'.pkl', 'rb') as file:
        collected_data.extend(pickle.load(file))

# Extract ciphertext values and traces from the loaded data
ciphertext_range = [entry['ciphertext'] for entry in collected_data]
traces = [entry['power_trace'] for entry in collected_data]

# Generate all possible key guesses as bytes
key_guess_range = range(256)
key_guess_range_bytes = [bytes([key_guess]) for key_guess in key_guess_range]
...
for key_guess in key_guess_range_bytes:
    ...
    for ciphertext in ciphertext_range:
        # Apply Inverse Shift Rows
        state = inverse_shift_rows(ciphertext)
        
        # S-box lookup after Inverse Shift Rows
        s_box_lookup = lookup(int.from_bytes(state, 'big') ^ int.from_bytes(key_guess, 'big'))
        hamming_power.append(get_hamming_weight(s_box_lookup))
        pos1 += 1

    # Process actual power array for each data point
    for trace in traces:
        actual_power = [trace_point for trace_point in trace]
        ...
        print(f"Key Guess {key_guess.hex()}: Pearson Coefficient {pearson_coefficient}")
